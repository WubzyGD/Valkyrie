...
module.exports = {
    ...
    meta: {
        category: 'Developer',
        description: "Completely blocks a user or server from using Valkyrie!",
        syntax: '`blacklist <user|guild> <add|delete> [@mention|ID]`',
        extra: null
    },
    help: "Disables a user from using Valkyrie (Usage: `{{p}}blacklist <user|guild> <add|delete> [@mention|ID])`",
    async execute(message, msg, args, cmd, prefix, mention, client) {
        ...
        if (['g', 'guild'].includes(args[0].toLowerCase())) {
            if (!tu || !tu.admin) {return message.channel.send('Sorry... you have to be a Valkyrie Admin to do this!');}
