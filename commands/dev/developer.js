...
module.exports = {
    ...
    help: new Discord.MessageEmbed()
        ...
        .setDescription("Add or remove users as Valkyrie developers.")
        .addField("Syntax", "`developer <add|remove> <@user|userID>`")
        .addField("Notice", "You must already be a developer of Valkyrie in order to use this command."),
    meta: {
        category: 'Developer',
        description: "Add or remove users as Valkyrie developers",
    ...
    async execute(message, msg, args, cmd, prefix, mention, client) {
        ...
        if (['c', 'check'].includes(args[0])) {return message.reply(`${person ? person : message.member.displayName} ${tu.developer ? 'is' : 'is not'} a Valkyrie developer.`);}
