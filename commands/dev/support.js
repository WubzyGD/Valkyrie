...
module.exports = {
    ...
    help: new Discord.MessageEmbed()
        ...
        .setDescription("Make a user a Valkyrie Support Team member")
        .addField("Syntax", "`supportstaff <add|remove|check> <@user|userID>`")
        .addField("Notice", "This command is only available to Valkyrie admin."),
    meta: {
        category: 'Developer',
        description: "Add or remove users as Valkyrie support",
    ...
    async execute(message, msg, args, cmd, prefix, mention, client) {
        ...
        if (['c', 'check'].includes(args[0])) {return message.reply(`${person ? person : message.member.displayName} ${tu.support ? 'is' : 'is not'} a part of Valkyrie Support.`);}
