...
module.exports = {
    ...
    help: new Discord.MessageEmbed()
        .setTitle("Help -> Staff")
        .setDescription("Make a user a Valkyrie staff member")
        .addField("Syntax", "`staff <add|remove|check> <@user|userID>`")
        .addField("Notice", "This command is only available to Valkyrie developers."),
    meta: {
        category: 'Developer',
        description: "Add or remove users as Valkyrie staff",
    ...
    async execute(message, msg, args, cmd, prefix, mention, client) {
        ...
        if (['c', 'check'].includes(args[0])) {return message.reply(`${person ? person : message.member.displayName} ${tu.staff ? 'is' : 'is not'} a part of Valkyrie Staff.`);}
        ...
        const logemb = (act) => new Discord.MessageEmbed()
            ...
            .setTimestamp();
        client.guilds.cache.get('679127746592636949').channels.cache.get('940432676802953216').send(logemb(['a', 'add'].includes(args[0]) ? 'Added' : 'Removed'));
