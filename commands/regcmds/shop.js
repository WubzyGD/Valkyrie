...
module.exports = {
    ...
    async execute(message, msg, args, cmd, prefix, mention, client) {
		...
		message.delete();
		if (!args.length) {var mr = await message.channel.send(`Syntax: \`${prefix}shop <fighter|f|prestigefighter|pf>\` or use \`${prefix}shop display\` to view your prices. **The shop does not have a purchase confirmation. If you use a buy command with sufficient funds, the items will be purchased.**`); return mr.delete({timeout: 20000});};
		var pstats = await userGameData.findOne({where: {user_id: message.author.id}});
		if (!pstats) {var mr = await message.reply("Hmm, it looks like you aren't in my databse. Send some messages and try again in a few minutes?"); return mr.delete({timeout: 20000});};
		...
		if (args[0] == "fighter" || args[0] == "f") {
			if (fighterCost > pstats.money) {var mr = await message.reply(`You don't have Gold enough for that! (\`${pstats.money}\`/\`${fighterCost}\`)`); return mr.delete({timeout: 20000});};
			...
			var mr = await message.reply("Fighter bought!"); mr.delete({timeout: 20000});
			...
			if (pfcost > pstats.lost_remnants) {var mr = await message.reply(`You don't have enough Lost Remnants for that! (\`${pstats.lost_remnants}\`/\`${pfcost}\`)`); return mr.delete({timeout: 20000});};
			...
			var mr = await message.reply("Prestige Fighter bought!"); mr.delete({timeout: 20000});
			...
			return mr.delete({timeout: 20000});
        } else {var mr = await message.reply(`Syntax: \`${prefix}shop <fighter|f|prestigefighter|pf>\` or use \`${prefix}shop display\` to view your prices.`); return mr.delete({timeout: 20000});};
    }
};