            ...
            async function twoAv() {
                if (!mention) {
                    if (args[2] == "flip") {
                        ...
                        msg.replace(/{{1}}/gm, message.member.guild.members.get(client.user.id).displayName);
                        msg.replace(/{{2}}/gm, message.member.displayName);
                    } else {
                        pfp1 = await Canvas.loadImage(message.author.avatarURL);
                        pfp2 = await Canvas.loadImage(client.user.avatarURL);
                        msg.replace(/{{2}}/gm, message.member.guild.members.get(client.user.id).displayName);
                        msg.replace(/{{1}}/gm, message.member.displayName);
                    };
                    ...
                    if (args[2] == "flip") {
                        ...
                        msg.replace(/{{1}}/gm, message.member.guild.members.get(message.mentions.users.first(2)[1].id).displayName);
                        msg.replace(/{{2}}/gm, message.member.guild.members.get(message.mentions.users.first().id).displayName);
                    } else {
                        ...
                        msg.replace(/{{2}}/gm, message.member.guild.members.get(message.mentions.users.first(2)[1].id).displayName);
                        msg.replace(/{{1}}/gm, message.member.guild.members.get(message.mentions.users.first().id).displayName);
                    };
                } else {
                    if (args[2] == "flip") {
                        pfp2 = await Canvas.loadImage(message.author.avatarURL);
                        pfp1 = await Canvas.loadImage(mention.avatarURL);
                        msg.replace(/{{2}}/gm, message.member.guild.members.get(mention.id).displayName);
                        msg.replace(/{{1}}/gm, message.member.displayName);
                    } else {
                        pfp1 = await Canvas.loadImage(message.author.avatarURL);
