...
module.exports = {
    ...
    execute(message, msg, args, cmd, prefix, mention, client) {
		...
		.addField("Members Online", `${message.member.guild.members.filter(m => m.presence.status == "online" && !m.user.bot).size} Members currently online.`)
		...
		.addField("Owner", `${message.author.username}#${client.users.find(u => u.id == message.member.guild.ownerID).discriminator}\nServer Username: ${message.member.guild.members.get(message.member.guild.ownerID).displayName}`)
