    ...
    async execute(message, msg, args, cmd, prefix, mention, client) {
        if (!args.length) {
            ...
            let category; for (category of Object.keys(sorted)) {
                let categorySorted = [];
                let current = 1;
                let currentEmbed = new Discord.MessageEmbed().setAuthor({name: "Help Menu", iconURL: message.author.displayAvatarURL()}).setTitle(category).setDescription("React to control the menu! You can also specify a command name when doing the help command to get more info about it.").setColor("dc134c");
                ...
                let command; for (command of commands) {
                    ...
                    if (current === 5) {
                        ...
                        current = 1;
                        currentEmbed = new Discord.MessageEmbed().setAuthor({name: "Help Menu", iconURL: message.author.displayAvatarURL()}).setTitle(category).setDescription("React to control the menu! You can also specify a command name when doing the help command to get more info about it.").setColor("dc134c");
                    }
                }
            ...
            } else {return message.channel.send({embeds: [pages[0].setFooter({text: "Valkyrie", iconURL: client.user.displayAvatarURL()}).setTimestamp()]}).catch(() => {});}
        } else {
            let command;
            ...
            return message.reply(command.help
                ? command.help instanceof Discord.MessageEmbed
                    ? {embeds: [command.help.setFooter({text: "Valkyrie | <required> [optional]", iconURL: client.user.displayAvatarURL()}).setColor("dc134c").setTimestamp()]}
