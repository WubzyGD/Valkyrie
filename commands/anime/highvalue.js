...
module.exports = {
    ...
    help: "Mark a character as high value. Must be a member of Valkyrie staff to do this.",
    async execute(message, msg, args, cmd, prefix, mention, client) {
        let tu = await UserData.findOne({uid: message.author.id});
        if (!tu || !tu.staff) {return message.channel.send("You cannot do this unless you are a member of Valkyrie staff.");}
        if (!args[0]) {
