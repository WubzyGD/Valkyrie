...
module.exports = {
    ...
    async execute(message, msg, args, cmd, prefix, mention, client) {
        ...
        if (['a', 'add', 'n', 'new'].includes(args[0].trim().toLowerCase())) {
            ...
            if (!tu || !tu.staff) {
                await message.channel.send("Since you aren't a Valkyrie Staff member, this anime will be __submitted__ for reviewal!");
                queue = true;
            }
        ...
        if (['e', 'edit'].includes(args[0].toLowerCase())) {
            args.shift();
            let tu = await UserData.findOne({uid: message.author.id});
            if (!tu || !tu.staff) {return await message.channel.send("Since you aren't a Valkyrie Staff member, you can't edit anime.");}
        ...
        if (['reject'].includes(args[0].trim().toLowerCase())) {
            let tu = await UserData.findOne({uid: message.author.id});
            if (!tu || !tu.staff) {await message.channel.send("Since you aren't a Valkyrie Staff member, you can't reject anime submissions!");}
        ...
        if (['accept', 'approve'].includes(args[0].toLowerCase())) {
            let tu = await UserData.findOne({uid: message.author.id});
            if (!tu || !tu.staff) {await message.channel.send("Since you aren't a Valkyrie Staff member, you can't accept anime submissions!");}
            let tr = await AniData.findOne({id: args[1].toLowerCase()});
            if (!tr) {return message.reply("That anime submission doesn't seem to exist!");}
            ...
            .then(() => {
                ...
                client.guilds.cache.get('762707532417335296').channels.cache.get('932177823630762014').send({embeds: [new Discord.MessageEmbed()
                    ...
                    .setDescription(`${tr.name} has been approved, and is now available to all Valkyrie users.`)
                    .setColor('dc134c')
        ...
        if (['w', 'watched'].includes(args[0].toLowerCase())) {
            ...
            if (args[0] && ['v', 'view'].includes(args[0].toLowerCase())) {
                ...
                } else {return message.channel.send({embeds: [pages[0].setFooter({text: "Valkyrie"})]});}
            }
