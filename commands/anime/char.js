...
module.exports = {
    ...
    async execute(message, msg, args, cmd, prefix, mention, client) {
        ...
        if (['a', 'add', 'n', 'new'].includes(args[0])) {
            ...
            if (!tu || !tu.staff) {
                await message.channel.send("Since you aren't a Valkyrie Staff member, this character will be __submitted__ for reviewal!");
                queue = true;
            }
        ...
        if (['e', 'edit'].includes(args[0].toLowerCase())) {
            args.shift();
            let tu = await UserData.findOne({uid: message.author.id});
            if (!tu || !tu.staff) {return await message.channel.send("Since you aren't a Valkyrie Staff member, you can't edit characters.");}
        ...
        if (['reject'].includes(args[0].trim().toLowerCase())) {
            let tu = await UserData.findOne({uid: message.author.id});
            if (!tu || !tu.staff) {await message.channel.send("Since you aren't a Valkyrie Staff member, you can't reject character submissions!");}
        ...
        if (['accept', 'approve'].includes(args[0].toLowerCase())) {
            let tu = await UserData.findOne({uid: message.author.id});
            if (!tu || !tu.staff) {await message.channel.send("Since you aren't a Valkyrie Staff member, you can't accept character submissions!");}
            let tr = await Char.findOne({id: args[1].toLowerCase()});
            if (!tr) {return message.reply("That character submission doesn't seem to exist!");}
        ...
        if (['la', 'listani', 'listanime', 'inani', 'inanime', 'ia'].includes(args[0].toLowerCase())) {
            ...
            if (paginate) {
                ...
                chs.forEach(ch => pages.push( new Discord.MessageEmbed()
                    ...
                    .addField("Loved by", `**${ch.loved}** Valkyrie user${ch.loved === 1 ? '' : 's'}!\n\`${prefix}char love ${ch.name}\``)
                    .setColor("dc134c")
