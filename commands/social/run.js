...
module.exports = {
    ...
    async execute(message, msg, args, cmd, prefix, mention, client) {
        ...
        if (['s', 'save', 'n', 'new', 'a', 'add'].includes(args[0].toLowerCase())) {
            ...
            if ((!tu || !tu.staff) && !client.misc.savers.includes(message.author.id)) {return message.reply("You must be a Valkyrie Staff member in order to add new running GIFs.");}
            let e = true;
            let id;
